...
settings_file="$HOME/.config/hypr/lua/settings.lua"
...
if [ ! -f "$layout_file" ]; then
  ...
  default_layout=$(grep 'kb_layout' "$settings_file" | head -1 | sed 's/.*=\s*"\?//; s/".*//; s/,.*//; s/[[:space:]]//g' 2>/dev/null)
...
if [ -f "$settings_file" ]; then
  kb_layout_line=$(grep 'kb_layout' "$settings_file" | head -1 | sed 's/.*=\s*//; s/["[:space:]]//g; s/,$//')
