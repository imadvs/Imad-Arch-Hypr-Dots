...
waybar_config="$HOME/.config/waybar/configs/TOP-Default-Laptop-IMAD"
...
if lspci -k | grep -A 2 -E "(VGA|3D)" | grep -iq nvidia; then
  ...
  # no hardware cursors if nvidia detected (not needed on modern drivers)
...
if hostnamectl | grep -q 'Chassis: vm'; then
  ...
  sed -i 's/^\([[:space:]]*no_hardware_cursors[[:space:]]*=[[:space:]]*\).*/\1 1/' config/hypr/configs/SystemSettings.conf
...
if [ -d ../assets/applications ]; then
    printf "\n%.0s" {1..1}
    echo "${NOTE} Copying Custom Desktop Files..."
    mkdir -p ~/.local/share/applications
    cp -r ../assets/applications/*.desktop ~/.local/share/applications/ 2>&1 | tee -a "$LOG"
    echo "${OK} Custom Desktop Files copied." 2>&1 | tee -a "$LOG"
fi
